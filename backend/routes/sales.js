...
router.post('/', authMiddleware, (req, res) => {
  try {
    ...
    const createSale = db.transaction(() => {
      ...
      db.prepare(
        'INSERT INTO sales (id, dateTime, items, totalAmount, createdBy, sessionId) VALUES (?, ?, ?, ?, ?, ?)'
      ).run(saleId, dateTime, JSON.stringify(validatedItems), totalAmount, req.user.id, sessionId || null);
    ...
    const result = { ...sale, items: JSON.parse(sale.items) };
